...
export default function CartDrawer() {
  ...
  const { items, subtotal, itemCount, remove, updateQuantity, isOpen, closeCart: onClose } = useCart();
      ...
      {isOpen && (
        <>
          ...
          >
            <div className="flex items-center justify-between px-8 pt-8 pb-5">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  {itemCount > 0 && (
                    <span className="absolute -top-1.5 -right-1.5 size-4 rounded-full bg-brand text-white text-[10px] font-semibold flex items-center justify-center">
                      {itemCount}
                    </span>
                  )}
                </div>
            ...
            {items.length > 0 && (
              <div className="px-8 pb-4">
                <div className="text-[12px] text-black/60 mb-2">
                  {remaining > 0 ? (
                    <>
                      Add <span className="font-semibold text-black">{formatPrice(remaining)}</span>{" "}
              ...
              {items.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center py-16 gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <p className="text-[16px] font-semibold">Your bag is empty</p>
            ...
            {items.length > 0 && (
              <div className="p-8 border-t border-black/5 bg-[#f5f5f5]">
                <div className="flex flex-col gap-3 mb-6">
                  <div className="flex items-center justify-between text-[13px]">
                    <span className="text-black/50 font-medium">Subtotal</span>
                    <span className="font-semibold text-black tabular-nums">{formatPrice(subtotal)}</span>
                  </div>
                  <div className="flex items-center justify-between text-[13px]">
                    <span className="text-black/50 font-medium">Shipping</span>
                    <span
                      className={`font-semibold tabular-nums ${
                        shipping === 0 ? "text-brand uppercase text-[11px] tracking-wider" : ""
                      }`}
                    >
                  ...
                  <div className="flex items-center justify-between text-[16px] pt-3 border-t border-black/10">
                    ...
                    <span className="font-semibold tracking-tight tabular-nums">{formatPrice(total)}</span>
                  </div>
                </div>

                <button
                  onClick={goToCheckout}
                  className="w-full bg-black text-white h-[56px] rounded-full flex items-center justify-between px-8 font-semibold hover:bg-brand transition-all group btn-press"
                >
                  <span>Checkout</span>
                  <ArrowRight
                    size={18}
